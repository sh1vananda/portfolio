...
const NavLink = ({ id, label, activeSection, scrollToSection, isMobile }) => (
  <li>
    <button
      ...
      className={`capitalize transition-colors duration-300 ${ isMobile ? 'block w-full text-left p-3' : 'pb-1'} ${ activeSection === id ? 'text-blue-500' : 'text-slate-500 dark:text-slate-400 hover:text-blue-500'}`}
    >
      {label}
    </button>
  </li>
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm border-b border-slate-200 dark:border-slate-800">
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        <div className="font-bold text-xl text-slate-800 dark:text-slate-200">S</div>
        <nav className="hidden md:flex items-center space-x-8">
          ...
          <button onClick={() => setIsDarkMode(!isDarkMode)} className="text-slate-500 dark:text-slate-400 hover:text-blue-500 transition-colors">
            {isDarkMode ? <SunIcon /> : <MoonIcon />}
          </button>
        </nav>
        <div className="md:hidden flex items-center">
            <button onClick={() => setIsDarkMode(!isDarkMode)} className="text-slate-500 dark:text-slate-400 mr-4 hover:text-blue-500 transition-colors">
                {isDarkMode ? <SunIcon /> : <MoonIcon />}
            </button>
...
const Section = ({ id, title, children }) => {
  ...
  return (
    <section id={id} ref={ref} className={`py-16 md:py-24 transition-opacity duration-700 ease-in ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <h2 className="text-3xl font-light text-slate-800 dark:text-slate-200 mb-10 text-center">{title}</h2>
      {children}
    </section>
  );
};
...
const ProjectCard = ({ project }) => (
    <div className="bg-white dark:bg-slate-800/50 p-6 rounded-lg border border-slate-200 dark:border-slate-700/50 group hover:border-blue-500 transition-colors duration-300">
        <h3 className="text-xl font-semibold text-slate-800 dark:text-slate-200 mb-2">{project.name}</h3>
        <p className="text-blue-500 mb-3 text-sm">{project.technologies.join(' • ')}</p>
        <p className="text-slate-600 dark:text-slate-400 text-base font-light">{project.description}</p>
    </div>
);
...
const App = () => {
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-900 font-sans text-slate-700 dark:text-slate-300 transition-colors duration-300">
      ...
      <main className="container mx-auto px-6 max-w-4xl">
        <section id="about" className="min-h-screen flex flex-col justify-center items-center text-center pt-24">
          <h1 className="text-4xl md:text-6xl font-bold text-slate-800 dark:text-slate-200 mb-3 tracking-tight">{resumeData.name}</h1>
          ...
          <div className="flex space-x-4">
            ...
            <button onClick={() => scrollToSection('projects')} className="px-6 py-3 bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-md hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">View Projects</button>
          </div>
        </section>
        ...
        <Section id="skills" title="Technical Skills">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {Object.entries(resumeData.skills).map(([category, items]) => (
                    <div key={category} className="bg-white dark:bg-slate-800/50 p-6 rounded-lg border border-slate-200 dark:border-slate-700/50">
                        <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4 capitalize">{category.replace(/([A-Z])/g, ' $1')}</h3>
                        <div className="flex flex-wrap gap-2">
                        {items.map((skill) => (
                            <span key={skill} className="text-sm bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-3 py-1 rounded-full">{skill}</span>
                        ))}
                        </div>
                    </div>
                ))}
        ...
        <Section id="experience" title="Professional Experience">
          <div className="space-y-8 relative before:absolute before:inset-0 before:ml-5 before:h-full before:w-0.5 before:bg-slate-200 dark:before:bg-slate-700">
              ...
              <div key={index} className="relative pl-10">
                ...
                <div className="pl-4">
                  <div className="flex justify-between items-baseline">
                    <h3 className="text-xl font-semibold text-slate-800 dark:text-slate-200">{job.position}</h3>
                  ...
                  <ul className="space-y-2 list-disc list-inside text-slate-600 dark:text-slate-400 font-light">
        ...
        <Section id="contact" title="Get In Touch">
            <div className="max-w-xl mx-auto text-center">
                ...
                <div className="flex justify-center space-x-6">
                    <a href={resumeData.linkedin} target="_blank" rel="noopener noreferrer" className="text-slate-500 dark:text-slate-400 hover:text-blue-500 transition-colors"><LinkedInIcon /></a>
                    <a href={resumeData.github} target="_blank" rel="noopener noreferrer" className="text-slate-500 dark:text-slate-400 hover:text-blue-500 transition-colors"><GitHubIcon /></a>
                </div>
            </div>
        </Section>
      </main>
      <footer className="text-center py-8 border-t border-slate-200 dark:border-slate-800 mt-16">
        <p className="text-sm text-slate-500 dark:text-slate-400">Designed & Built by Shivananda Reddy Kankanala</p>
      </footer>
    </div>
  );
};
